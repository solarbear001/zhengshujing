...
const Blog = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      {/* Header — inverted for white bg */}
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 py-6 bg-white/80 backdrop-blur-sm">
        <Link to="/" className="font-mono text-xs tracking-[0.3em] text-black uppercase">
          SHUJING ZHENG
        </Link>
        <nav className="flex items-center gap-6 md:gap-8">
          {[
            ...
            <Link
              ...
              className="font-mono text-xs tracking-[0.2em] text-black/50 hover:text-black transition-colors duration-300 uppercase"
            >
              {t(item.en, item.zh)}
            </Link>
          ))}
          <button
            onClick={toggleLang}
            className="font-mono text-[10px] tracking-[0.2em] text-black/50 hover:text-black transition-colors duration-300 uppercase border border-black/20 px-3 py-1"
          >
            {lang === "en" ? "中文" : "EN"}
          </button>
        </nav>
        ...
        <section className="px-6 md:px-20 pb-32">
          <div className="max-w-5xl mx-auto space-y-0">
            {posts.map((post, i) => (
              ...
              >
                {/* Left: Letter + Category + Date */}
                <div className="col-span-12 md:col-span-2">
                  <span
                    className="text-display text-black/8 leading-none block"
                  >
                    {post.letter}
                  </span>
                ...
                <div className="col-span-12 md:col-span-7 md:col-start-4 flex flex-col justify-between">
                  ...
                  <a
                    href={post.link}
                    className="inline-block mt-6 font-mono text-[10px] tracking-[0.3em] text-black/40 hover:text-black transition-colors duration-300 uppercase"
                  >
                    {t("READ →", "阅读 →")}
                  </a>
                </div>
      ...
      {/* Footer — inverted */}
      <footer className="px-6 md:px-20 py-16 border-t border-black/10">
        <div className="max-w-5xl mx-auto grid grid-cols-3 gap-8">
          <div>
            <a
              ...
              className="font-mono text-[10px] tracking-[0.3em] text-black/40 hover:text-black transition-colors uppercase"
            >
              LinkedIn ↗
            </a>
          </div>
          <div className="text-center space-y-2">
            ...
            <a
              href="mailto:hello@shujingzheng.com"
              className="font-mono text-[10px] tracking-[0.2em] text-black/60 hover:text-black transition-colors"
            >
