      ...
      <main>
        ...
        <section className="min-h-[70vh] flex flex-col justify-center px-6 md:px-20 pt-32 pb-16">
          <div className="max-w-5xl w-full space-y-6">
            <h1 className="text-display tracking-tight text-black">{t("bl g", "观 察")}</h1>
            <p
              className="text-black/30"
              style={{
                fontFamily: "Georgia, '黑体', sans-serif",
        ...
        <section className="px-6 md:px-20 pb-32">
          <div className="max-w-6xl mx-auto space-y-0">
            {posts.map((post, i) => {
              const isEven = i % 2 === 0;
              return (
                <article key={i} className="grid grid-cols-12 gap-6 md:gap-8 py-20 border-t border-black/10 group">
                  {isEven ? (
                    <>
                      {/* Left-aligned: Letter + meta */}
                      <div className="col-span-12 md:col-span-2">
                        <span className="text-display text-black/[0.06] leading-none block">{post.letter}</span>
